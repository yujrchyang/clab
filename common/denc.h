...
// populate object-corpus.  Note that there is an almost
...
public:
    DencDumper(const char *name,
               const bufferlist::contiguous_appender &appender)
        : name{name},
          appender{appender},
...
private:
    ...
    size_t space_size() const {
        return appender.get_logical_offset() -
            appender.get_out_of_band_offset();
    }
    void dump() const {
        char fn[PATH_MAX];
        ::snprintf(fn, sizeof(fn),
                   ...
                   getpid(), i++);
        int fd = ::open(fn, O_WRONLY | O_TRUNC | O_CREAT | O_CLOEXEC,
                        0644);
        if (fd < 0) {
            return;
        }
...
template <typename T>
inline void denc_varint(T v, buffer::list::contiguous_appender &p) {
    uint8_t byte = v & 0x7f;
    v >>= 7;
    while (v) {
        byte |= 0x80;
...
template <typename T>
inline void denc_varint_lowz(T &v, buffer::ptr::const_iterator &p) {
    uint64_t i = 0;
    denc_varint(i, p);
    int lowznib = (i & 3);
    i >>= 2;
...
template <typename T>
struct has_legacy_denc<T, decltype(std::declval<T &>().decode(std::declval<buffer::list::const_iterator &>()))>
    : std::true_type {
    static void decode(T &v, buffer::list::const_iterator &p) {
        v.decode(p);
    }
};
template <typename T>
...
//
template <>
struct denc_traits<buffer::ptr> {
    static constexpr bool supported = true;
    static constexpr bool featured = false;
    static constexpr bool bounded = false;
    static constexpr bool need_contiguous = false;
    static void bound_encode(const buffer::ptr &v, size_t &p, uint64_t f = 0) {
        p += sizeof(uint32_t) + v.length();
    }
    template <class It>
    static std::enable_if_t<!is_const_iterator_v<It>>
    encode(const buffer::ptr &v, It &p, uint64_t f = 0) {
        denc((uint32_t)v.length(), p);
        p.append(v);
    }
    template <class It>
    static std::enable_if_t<is_const_iterator_v<It>>
    decode(buffer::ptr &v, It &p, uint64_t f = 0) {
        uint32_t len;
        denc(len, p);
        v = p.get_ptr(len);
    }
    static void decode(buffer::ptr &v, buffer::list::const_iterator &p) {
        uint32_t len;
        denc(len, p);
        buffer::list s;
        p.copy(len, s);
...
//
template <>
struct denc_traits<buffer::list> {
    static constexpr bool supported = true;
    static constexpr bool featured = false;
    static constexpr bool bounded = false;
    static constexpr bool need_contiguous = false;
    static void bound_encode(const buffer::list &v, size_t &p, uint64_t f = 0) {
        p += sizeof(uint32_t) + v.length();
    }
    static void encode(const buffer::list &v,
                       buffer::list::contiguous_appender &p,
                       uint64_t f = 0) {
        denc((uint32_t)v.length(), p);
        p.append(v);
    }
    static void decode(buffer::list &v, buffer::ptr::const_iterator &p,
                       uint64_t f = 0) {
        uint32_t len;
        denc(len, p);
        v.clear();
    ...
    static void decode(buffer::list &v, buffer::list::const_iterator &p) {
        uint32_t len;
        denc(len, p);
        v.clear();
        p.copy(len, v);
    }
    static void encode_nohead(const buffer::list &v,
                              buffer::list::contiguous_appender &p) {
        p.append(v);
    }
    static void decode_nohead(size_t len, buffer::list &v,
...
struct denc_traits<
    std::pair<A, B>,
    std::enable_if_t<denc_supported<std::remove_const_t<A>> &&
                     denc_supported<B>>> {
    ...
    static void bound_encode(const std::pair<A, B> &v, size_t &p,
                             uint64_t f = 0) {
    ...
    static void encode(const std::pair<A, B> &v,
                       buffer::list::contiguous_appender &p,
                       uint64_t f = 0) {
        if constexpr (featured) {
            denc(v.first, p, f);
    ...
    static void decode(std::pair<A, B> &v, buffer::ptr::const_iterator &p,
                       uint64_t f = 0) {
...
public:
    ...
    static void decode(container &s, buffer::ptr::const_iterator &p,
                       uint64_t f = 0) {
        uint32_t num;
        denc(num, p);
...
// Must be invoked from inside namespace TOPNSPC, or the type T must be
// fully qualified to match the namespace where denc_traits lives.
#define _DECLARE_CLASS_DENC(T, b)                                         \
    ...
    struct denc_traits<T> {                                               \
        ...
        static constexpr bool need_contiguous =                           \
            !_denc::has_legacy_denc<T>::value;                            \
        static void bound_encode(const T &v, size_t &p, uint64_t f = 0) { \
            v.bound_encode(p);                                            \
        }                                                                 \
        static void encode(const T &v,                                    \
                           buffer::list::contiguous_appender &p,          \
                           uint64_t f = 0) {                              \
            v.encode(p);                                                  \
        }                                                                 \
        static void decode(T &v, buffer::ptr::const_iterator &p,          \
                           uint64_t f = 0) {                              \
            v.decode(p);                                                  \
        }                                                                 \
    };
...
inline std::enable_if_t<traits::supported && !traits::need_contiguous> decode(
    ...
    if (!p.is_pointing_same_raw(bl.back()) && remaining > page().size) {
        traits::decode(o, p);
    } else {
        buffer::ptr tmp;
        auto t = p;
...
#define DENC_HELPERS                                               \
    ...
    static void _denc_finish(size_t &p,                            \
                             ...
                             __u8 *struct_compat,                  \
                             char **, uint32_t *) {}               \
    static void _denc_start(buffer::list::contiguous_appender &p,  \
                            __u8 *struct_v,                        \
                            __u8 *struct_compat,                   \
                            char **len_pos,                        \
    ...
    static void _denc_finish(buffer::list::contiguous_appender &p, \
                             __u8 *struct_v,                       \
                             __u8 *struct_compat,                  \
                             char **len_pos,                       \
                             uint32_t *start_oob_off) {            \
