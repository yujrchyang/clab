...
// ── fixed_to_string / fixed_u_to_string ─────────────────────────
...
FormatterAttrs::FormatterAttrs(const char *attr, ...) {
    ...
    do {
        ...
        if (!val) break;
        attrs.push_back(std::make_pair(std::string(s), std::string(val)));
        s = va_arg(ap, char *);
    } while (s);
    va_end(ap);
}
...
void Formatter::flush(buffer::list &bl) {
    std::stringstream os;
    flush(os);
    bl.append(os.str());
}
...
void Formatter::dump_format_ns(std::string_view name, const char *ns,
                               const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
...
void Formatter::dump_format_unquoted(std::string_view name,
                                     const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
...
void JSONFormatter::open_section(std::string_view name, const char *ns,
                                 bool is_array) {
...
void JSONFormatter::open_array_section_in_ns(std::string_view name,
                                             const char *ns) {
...
void JSONFormatter::open_object_section_in_ns(std::string_view name,
                                              const char *ns) {
...
void JSONFormatter::add_value(std::string_view name, std::string_view val,
                              bool quoted) {
...
void XMLFormatter::open_object_section_with_attrs(
    std::string_view name, const FormatterAttrs &attrs) {
    open_section_in_ns(name, NULL, &attrs);
}

void XMLFormatter::open_object_section_in_ns(std::string_view name,
                                             const char *ns) {
...
void XMLFormatter::open_array_section_with_attrs(
    std::string_view name, const FormatterAttrs &attrs) {
    open_section_in_ns(name, NULL, &attrs);
}

void XMLFormatter::open_array_section_in_ns(std::string_view name,
                                            const char *ns) {
    open_section_in_ns(name, ns, NULL);
}
...
void XMLFormatter::close_section() {
    cxxlab_assert(!m_sections.empty());
    finish_pending_string();
...
void XMLFormatter::dump_string_with_attrs(std::string_view name,
                                          std::string_view s,
    ...
    print_spaces();
    m_ss << "<" << e << attrs_str << ">" << xml_stream_escaper(s) << "</"
         << e << ">";
    if (m_pretty) m_ss << "\n";
}
...
void XMLFormatter::dump_format_va(std::string_view name, const char *ns,
                                  bool quoted, const char *fmt,
                                  va_list ap) {
    char buf[LARGE_SIZE];
    ...
    if (ns) {
        m_ss << "<" << e << " xmlns=\"" << ns << "\">"
             << xml_stream_escaper(std::string_view(buf, len)) << "</" << e
             << ">";
    } else {
        m_ss << "<" << e << ">" << xml_stream_escaper(std::string_view(buf, len))
             << "</" << e << ">";
    }
...
void XMLFormatter::get_attrs_str(const FormatterAttrs *attrs,
                                 std::string &attrs_str) {
    std::stringstream attrs_ss;

    for (auto &p : attrs->attrs) {
        attrs_ss << " " << p.first << "="
                 << "\"" << p.second << "\"";
    }
...
void XMLFormatter::open_section_in_ns(std::string_view name, const char *ns,
                                      const FormatterAttrs *attrs) {
    print_spaces();
    std::string attrs_str;
    ...
    for (size_t i = 0; i < m_vec.size(); i++) {
        ...
        for (size_t j = 0; j < m_vec[i].size(); j++) {
            ...
            if (m_keyval) {
                ...
                os << fmt::format("{:<{}}|", m_vec[i][j].second,
                                  m_column_size[j] + 2);
            }
        }
...
void TableFormatter::open_object_section_with_attrs(
    std::string_view name, const FormatterAttrs &attrs) {
    open_section_in_ns(name, NULL, NULL);
}

void TableFormatter::open_object_section_in_ns(std::string_view name,
                                               const char *ns) {
    open_section_in_ns(name, NULL, NULL);
}
...
void TableFormatter::open_array_section_with_attrs(
    std::string_view name, const FormatterAttrs &attrs) {
    open_section_in_ns(name, NULL, NULL);
}

void TableFormatter::open_array_section_in_ns(std::string_view name,
                                              const char *ns) {
    open_section_in_ns(name, NULL, NULL);
}

void TableFormatter::open_section_in_ns(std::string_view name, const char *ns,
                                        const FormatterAttrs *attrs) {
...
void TableFormatter::add_value(std::string_view name, T val) {
    ...
    m_vec[i].push_back(
        std::make_pair(get_section_name(name), m_ss.str()));
    m_ss.clear();
    m_ss.str("");
}
...
void TableFormatter::dump_string_with_attrs(std::string_view name,
                                            std::string_view s,
                                            const FormatterAttrs &attrs) {
    finish_pending_string();
    size_t i = m_vec_index(name);
...
void TableFormatter::dump_format_va(std::string_view name, const char *ns,
                                    bool quoted, const char *fmt,
                                    va_list ap) {
    finish_pending_string();
    char buf[LARGE_SIZE];
...
void TableFormatter::get_attrs_str(const FormatterAttrs *attrs,
                                   std::string &attrs_str) {
    std::stringstream attrs_ss;
...
void HTMLFormatter::set_status(int status, const char *status_name) {
    m_status = status;
    m_status_name = status_name ? status_name : "";
}

void HTMLFormatter::output_header() {
    if (!m_header_done) {
        m_header_done = true;
        std::string status_line = std::to_string(m_status);
        if (!m_status_name.empty()) {
            status_line += " ";
...
void HTMLFormatter::dump_string_with_attrs(
    std::string_view name, std::string_view s, const FormatterAttrs &attrs) {
    std::string e(name);
    std::string attrs_str;
    get_attrs_str(&attrs, attrs_str);
    print_spaces();
    m_ss << "<li>" << e << ": " << xml_stream_escaper(s) << attrs_str
         << "</li>";
    if (m_pretty) m_ss << "\n";
}
...
void HTMLFormatter::dump_format_va(std::string_view name, const char *ns,
                                   bool quoted, const char *fmt,
                                   va_list ap) {
    char buf[LARGE_SIZE];
    size_t len = vsnprintf(buf, LARGE_SIZE, fmt, ap);
